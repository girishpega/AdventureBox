...
const OUT = ".po-review/screenshots/v0.6.0";
...
const errors = [];
const report = { sprint: "v0.6.0", brand: "WonderBerry", screenshots: [], errors: [] };
...
// Stories 1-4 (first page)
for (let i = 0; i < STORIES.length; i++) {
  const slug = STORIES[i];
  const num = String(i + 1).padStart(2, "0");
  await capture(`05-story${num}-page1`, 1280, 900, `/stories/${slug}`);
}

// Mobile reader (story 1)
await capture("09-mobile-reader", 390, 844, `/stories/${STORIES[0]}`);
